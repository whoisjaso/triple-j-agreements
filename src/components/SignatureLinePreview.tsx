...
export default function SignatureLinePreview({ label, dateLabel = 'Date', signatureImage, signatureDate, printedName }: Props) {
  return (
    <div>
      <div className="border-b border-luxury-ink h-14 mb-2 relative flex items-end">
        {signatureImage && (
          <img
            src={signatureImage}
            alt={label}
            className="h-12 max-w-[200px] object-contain absolute bottom-0 left-2"
          />
        )}
        {signatureDate && (
          <span className="absolute bottom-1 right-0 text-xs text-luxury-ink/70 font-medium">
      ...
      {printedName && <p className="text-[10px] text-luxury-ink/40 mt-1">{printedName}</p>}
    </div>
  );
}
